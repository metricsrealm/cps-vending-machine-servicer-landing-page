...
export default function FAQ() {
  ...
  return (
    <section className="bg-tint py-[78px]" id="home-faq-section">
      <div className="max-w-[1200px] mx-auto px-6" id="home-faq-container">
        {/* Section Header */}
        <div className="max-w-[680px] mx-auto text-center mb-[46px]" id="home-faq-header">
          <span className="font-display font-bold text-[12px] tracking-[0.14em] uppercase text-orange" id="home-faq-tagline">
            Got questions?
          </span>
          <h2 className="font-display font-bold text-[27px] sm:text-[32px] md:text-[38px] text-navy mt-3 leading-[1.15] tracking-[-0.01em]" id="home-faq-heading">
            Frequently asked questions
          </h2>
          <p className="font-sans text-[#5B6472] text-[16.5px] leading-relaxed max-w-[500px] mx-auto" id="home-faq-description">
            Don't see your question? Call us — we're happy to help.
          </p>
        </div>

        {/* FAQ Accordion List */}
        <div className="max-w-[780px] mx-auto space-y-3" id="home-faq-accordion-list">
          {faqItems.map((item, idx) => {
            const isOpen = openId === item.id;
            return (
              <div
                key={item.id}
                id={`home-faq-card-${idx + 1}`}
              ...
              >
                ...
                <button
                  ...
                  id={`home-faq-question-btn-${idx + 1}`}
                ...
                >
                  <span id={`home-faq-question-text-${idx + 1}`}>{item.question}</span>
                  <svg 
                    width="20" 
                    height="20" 
                    viewBox="0 0 24 24" 
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-[22px] pb-[20px] pt-0 font-sans text-[#5B6472] text-[15px] leading-relaxed" id={`home-faq-answer-text-${idx + 1}`}>
                        {item.answer}
                      </div>
                    </motion.div>
                  )}
